### Protist + Fungi MetaT ###
### Function heatmap: WC vs. Net Trap T0 ###
...
zscore_fxn <- function(df,exp){
  ...
  df <- subset(df,grepl("Water",df$variable)| grepl("RotT0",df$variable))
...
all <- subset(all,grepl("Water",all$variable) | grepl("RotT0",all$variable))
kegg <- unique(all$KEGG)
out <- NULL

# Use loop to identify KOs that are always most expressed in one sample type vs. another
...
# Fetch KO term annotations
annotations <- lapply(out$v1, function(ko) {
  KEGGREST::keggGet(ko)
})

# Grab the names of the KO terms
t <- NULL
...
# Prep for plot
names <- data.frame(KEGG=c(out$v1),names=c(t),var=c(out$v2))
...
all_arr_rot <- all%>% filter(var==2) %>% arrange(desc(m))
all_arr_rot <- unique(all_arr_rot$KEGG)

all_arr_water <- all%>% filter(var==1) %>% arrange(desc(m))
all_arr_water <- unique(all_arr_water$KEGG)

# Plot
all %>% filter(KEGG %in% all_arr_rot[1:15] | KEGG %in% all_arr_water[1:14]) %>% 
  ggplot(aes(x=Sample,y=reorder(names,m),fill=m))+
...
ggsave("../../Figure4.png",width=16,height=8)
